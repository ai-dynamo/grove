...
- [Proposal](#proposal)
  ...
  - [Step-by-step PodGang generation](#step-by-step-podgang-generation)
    - [The algorithm](#the-algorithm)
    - [Why this works](#why-this-works)
    - [Example PCS used in the walkthroughs](#example-pcs-used-in-the-walkthroughs)
    - [Example 1 — 10F, 11P, 13D, <code>minAvailable</code> = (2F, 2P, 2D)](#example-1--10f-11p-13d-minavailable--2f-2p-2d)
    - [Example 2 — 4F, 100P, 150D, <code>minAvailable</code> = (2F, 2P, 3D)](#example-2--4f-100p-150d-minavailable--2f-2p-3d)
    - [Standalone PCLQ surplus](#standalone-pclq-surplus)
...
- [Design Details](#design-details)
  - [API Changes](#api-changes)
    ...
    - [Labels on <code>PodGang</code> resources](#labels-on-podgang-resources)
  ...
  - [Coherent update behavior and flow](#coherent-update-behavior-and-flow)
    ...
    - [<code>DependsOn</code> and scheduling order](#dependson-and-scheduling-order)
      - [Labels carried by every PodGang](#labels-carried-by-every-podgang)
      - [Selectors authored per PodGang role](#selectors-authored-per-podgang-role)
      - [Gate-removal evaluation](#gate-removal-evaluation)
      - [Steady-state scale-out](#steady-state-scale-out)
...
### Step-by-step PodGang generation

A `PodCliqueSet` replica's PodGangs are generated by a single algorithm that runs in **steps**. Each step emits one MPG and zero or more TPGs. The same algorithm drives two flows:

- **Initial deployment.** All steps are computed up front and the entire PodGangMap is written in one shot. Runtime ordering between steps is enforced by labels and `DependsOn` selectors carried on each PGM entry — no per-step write is needed.
- **Coherent update.** One step per iteration. The PodGangMap component computes step `N`'s entries, writes them to PGM, and waits until every PodGang created in that step reports `Available=True` before emitting step `N+1`.

#### The algorithm

For each updated component (every standalone `PodClique` and every `PodCliqueScalingGroup` in scope of the update), let:

- `replicas` — the desired pod count.
- `minAvailable` — the corresponding `MinAvailable` from the PCS spec.

We track `remaining` per component — pods of that component still to be placed. It starts equal to `replicas` and is decremented by everything emitted in each step.

At each step:

1. **Compute `numMPGs`** — how many MPGs can still be created from this step onwards, including the one created in this step. It is the minimum value of `floor(remaining / minAvailable)` across all components. Once any component runs out of room for another full MPG, no more MPGs can be created.
2. **Compute `remainder`** for each PCSG as `remaining − numMPGs × minAvailable`. These are PCSG replicas that won't be placed even if all `numMPGs` future MPGs run at `minAvailable` each. They have to be emitted as TPGs, spread across the remaining steps.
3. **Emit one MPG** containing `minAvailable` pods of every updated component.
4. **For every PCSG, emit `floor(remainder / numMPGs)` single-replica TPGs** in this step. Standalone PCLQs do not produce TPGs.
5. **Subtract the MPG and TPGs emitted in this step from `remaining`.**

The algorithm stops when `remaining` is zero for every component.

When the loop finishes, any standalone PCLQ pods left over (because `replicas` isn't a multiple of `minAvailable`) are folded into the last MPG of the run.

#### Why this works

- **No tail-only TPG steps.** In the final step, `numMPGs` is always 1, so `floor(remainder / 1) = remainder` — every leftover PCSG replica is emitted in that step. There is never a step that emits only TPGs and no MPG.
- **Bounded disruption during a coherent update.** Each step replaces exactly `minAvailable` of every component (one new MPG) plus a small batch of single-replica TPGs. The number of pods being rolled at once is bounded by the step shape, not by the total replica count.
- **Each step respects the current PCSG ratios** (when PCSGs are in scope). Standalone PCLQ pods always ride inside the step's MPG, so their placement is uniform across steps. PCSGs, on the other hand, ride in the MPG *plus* spread out as TPGs — and because `floor(remainder / numMPGs)` TPGs are emitted per PCSG every step, an over-provisioned PCSG drains in proportion to how over-provisioned it is. After every step, the placed PCSG-replica counts stay close to the ratio implied by the live `replicas`, so serving capacity reflects the current traffic shape rather than the original `MinAvailable` shape.
- **Uneven divisions don't lose pods.** When `remainder / numMPGs` doesn't divide evenly, only the floor of that division is emitted as TPGs in this step. The pods that would have rounded off are still in `remaining` going into the next step, so they get picked up by the next step's recomputation. No special bookkeeping is needed to carry residue forward.

#### Example PCS used in the walkthroughs

The two worked examples below assume the same `PodCliqueSet` shape — a typical disaggregated-inference setup with one standalone `PodClique` and two `PodCliqueScalingGroup`s:

- **Frontend** (abbreviated **F**) — a standalone `PodClique` that handles request ingress.
- **Prefill** (abbreviated **P**) — a `PodCliqueScalingGroup` for the prefill stage.
- **Decode** (abbreviated **D**) — a `PodCliqueScalingGroup` for the decode stage.

Each example varies the `replicas` and `minAvailable` values to illustrate different states the algorithm has to handle.

#### Example 1 — 10F, 11P, 13D, `minAvailable` = (2F, 2P, 2D)

| Step | `remaining` (F, P, D) | `numMPGs` | `remainder` (P, D) | Emitted | After step (F, P, D) |
|------|------------------------|-----------|---------------------|---------|----------------------|
| 1 | 10, 11, 13 | 5 | 1, 3 | MPG{2F, 2P, 2D}                         | 8, 9, 11 |
| 2 | 8, 9, 11   | 4 | 1, 3 | MPG{2F, 2P, 2D}                         | 6, 7, 9  |
| 3 | 6, 7, 9    | 3 | 1, 3 | MPG{2F, 2P, 2D} + 1×TPG{1D}             | 4, 5, 6  |
| 4 | 4, 5, 6    | 2 | 1, 2 | MPG{2F, 2P, 2D} + 1×TPG{1D}             | 2, 3, 4  |
| 5 | 2, 3, 4    | 1 | 1, 2 | MPG{2F, 2P, 2D} + 1×TPG{1P} + 2×TPG{1D} | 0, 0, 0  |

Total: 5 MPGs (10F, 10P, 10D) + 1 P-TPG + 4 D-TPGs = 10F, 11P, 13D.

#### Example 2 — 4F, 100P, 150D, `minAvailable` = (2F, 2P, 3D)

P and D have been scaled out heavily relative to F (the standalone). F is the bottleneck — only two MPGs fit.

| Step | `remaining` (F, P, D) | `numMPGs` | `remainder` (P, D) | Emitted | After step (F, P, D) |
|------|------------------------|-----------|---------------------|---------|----------------------|
| 1 | 4, 100, 150 | 2 | 96, 144 | MPG{2F, 2P, 3D} + 48×TPG{1P} + 72×TPG{1D} | 2, 50, 75 |
| 2 | 2, 50, 75   | 1 | 48, 72  | MPG{2F, 2P, 3D} + 48×TPG{1P} + 72×TPG{1D} | 0, 0, 0   |

Total: 2 MPGs + 96 P-TPGs + 144 D-TPGs = 4F, 100P, 150D.

The TPG counts each step are naturally proportional to how over-provisioned each PCSG is — the algorithm doesn't compute ratios explicitly.

#### Standalone PCLQ surplus

When a standalone PCLQ's `replicas` isn't a multiple of its `minAvailable`, the leftover pods are folded into the last MPG. For example, with `replicas[F] = 11` and `minAvailable[F] = 2`, the algorithm creates 5 MPGs (placing 10F across them) and the 11th F pod is added to the 5th MPG, making it `{3F, ...}`.
...
type PodGangEntry struct {
    ...
    // before pods in this PodGang have their scheduling gates removed. A nil selector means no
    // dependency. See [DependsOn and scheduling order] for the selector authoring rules.
    // +optional
...
`InFlightPodGangs` is the orchestrator's hand-off to the PodGangMap component and back. It also provides visibility into which PodGangs are currently being rolled.

A coherent update for a PodCliqueSet replica proceeds **one step at a time**, where each step emits one MPG and zero or more TPGs (see [Step-by-step PodGang generation](#step-by-step-podgang-generation)). The mechanism for advancing through steps is:

1. While `InFlightPodGangs` is empty, the PodGangMap component computes the next step's PGM entries — adding the new-hash MPG and any co-emitted TPGs, and decrementing the corresponding old-hash entries.
2. The orchestrator reads PGM, picks the new-hash entries that are not yet `Available`, and writes their names into `InFlightPodGangs`. For a step that emits one MPG and `k` TPGs, this is `1 + k` PodGang names.
3. The orchestrator waits for every listed PodGang to reach `PodGangConditionTypeAvailable=True`, then clears the field. This signals the PodGangMap component to compute the next step.
...
In addition to the standard PCS-managed-resource labels mirrored by the PodGang component (`grove.io/part-of`, `grove.io/component`, `grove.io/managed-by`, etc.), every `PodGang` resource managed by Coherent carries two coherent-update-specific labels:
...
**New behavior under Coherent.** New PCS deployments no longer produce a BPG/SPG layout. The PodGangMap component runs the [Step-by-step PodGang generation](#step-by-step-podgang-generation) algorithm against the full PCS template — every standalone PodClique and every PodCliqueScalingGroup is in scope, since at deploy time everything is "new" — and writes the resulting MPG and TPG entries to the replica's `PodGangMap` in **one shot**. There is no per-step write at deploy time; runtime ordering between steps is enforced by the labels and `DependsOn` selectors carried on each entry (see [`DependsOn` and scheduling order](#dependson-and-scheduling-order)).

The PodGang component then materializes a `PodGang` resource for each entry, with `MinReplicas` set to the gang's MinAvailable. Because the PGM is generated by the same algorithm that drives coherent updates, the initial layout and any post-update layout are structurally identical — bootstrap produces no shape that updates have to reshape later. PGM is therefore the single descriptor of desired PodGang composition across the entire lifecycle of a PCS replica: bootstrap, coherent update, and steady-state scale operations.

> **Migration note.** When the PodGangMap component first runs against a PCS replica that pre-dates this proposal — one already serving traffic with the legacy BPG/SPG shape — it reconstructs PGM entries from the live `PodGang` resources first, so the existing names and `DependsOn` graph are preserved. Only when no PodGangs exist for the replica does PGM fall back to computing entries from the PCS template. Legacy gangs are not torn down on first reconcile after a Grove upgrade; they get drained out naturally by subsequent coherent updates.
...
Grove encodes the MVU's gang-scheduling intent by generating new `PodGang` resources. Each step of an update emits one **MVU PodGang (MPG)** carrying the MVU's pods, plus zero or more **Tail PodGangs (TPGs)**, each carrying a single PCSG replica beyond what the MPGs alone can absorb. The number of MPGs across the update and the number of TPGs co-emitted in each step are determined by the [Step-by-step PodGang generation](#step-by-step-podgang-generation) algorithm. Each MPG is its own gang and each TPG is its own gang — they are scheduled independently. TPGs `DependsOn` the MPG of their own step (via a label selector — see [DependsOn and scheduling order](#dependson-and-scheduling-order)), so the scheduler places TPG pods only after their step's MPG reports `Available=True`.

A coherent update runs **one step at a time**. The PodGangMap component computes step `N`'s entries (MPG plus any co-emitted TPGs), and the orchestrator waits for every PodGang in step `N` to reach `Available=True` before step `N+1` is computed. Disruption is bounded by the step shape — at most one step's worth of new-hash pods is in flight at any moment.
...
3. **Gate on prior iteration availability.** The PodGangMap component will not emit the next iteration until every previously-generated new-hash PodGang reports `PodGangConditionTypeAvailable=True`. This is how "exactly one MVU in flight at a time" is realized.
...
6. **Wait for `Available`.** The orchestrator populates `Status.UpdateProgress.CurrentlyUpdating[0].InFlightPodGangs` with the names of new-hash PodGangs that are not yet Available, and waits on their `PodGangConditionTypeAvailable` condition. `MinReplicas` on a PodGang is a scheduling constraint — it instructs the backend scheduler to place at least that many pods of each PodGroup together before any of them run. Once at least `MinReplicas` pods of every PodGroup are ready, the PodGang component releases the constraint by setting `MinReplicas=0` on all PodGroups, then sets `Available=True`. Releasing the constraint is necessary because with multiple MPGs a future scale-in may reduce the pod count of a given MPG below its original `MinReplicas` without breaching `MinAvailable` at the PCLQ/PCSG level — leaving `MinReplicas` intact would cause the backend scheduler to terminate the gang even though the `MinAvailable` guarantee for the PCLQs and PCSGs in the PCS spec is still met.

7. **Advance.** Once all in-flight PodGangs are Available, the orchestrator clears `InFlightPodGangs` and explicitly requeues (the status-only patch does not fire a new reconcile on its own since the PCS For-watch uses `GenerationChangedPredicate`). On the next reconcile the PodGangMap component computes the next iteration's entries (step 2). When no old-hash entries remain for the replica, the orchestrator marks the replica's `UpdateEndedAt` and seeds `CurrentlyUpdating[0]` for the next pending replica. When all replicas are done, `UpdateProgress.UpdateEndedAt` is set and the strategy enters steady state.
...
At t=0 there is no update in flight. The PodGangMap component runs the [Step-by-step PodGang generation](#step-by-step-podgang-generation) algorithm against the PCS template and writes all step entries to `PodGangMap` in one shot. The PodGang component then materializes the corresponding `PodGang` resources.
...
sequenceDiagram
  ...
  PGM->>PGM: run step-by-step generation across full PCS template, write all entries to PGM in one shot
  PG->>PG: create PodGangs from PGM with MinReplicas set and labels stamped
  ...
  PCLQ->>PCLQ: create standalone PCLQ pods schedule-gated against PodGangs
  PCSG->>PCSG: create PCSG-owned PCLQ pods (no scheduling gate)
  PCLQ->>PCLQ: remove scheduling gates once PodGang is Initialized and all DependsOn PodGangs are Initialized
  PG->>PG: pods ready, then release MinReplicas to 0 and set PodGang Available true
```

##### One coherent update iteration

An iteration corresponds to one step of the [Step-by-step PodGang generation](#step-by-step-podgang-generation) algorithm. The PodGangMap component computes the step's MPG and any co-emitted TPGs together, and the orchestrator gates the next iteration on every PodGang in this step reaching `Available=True`.
...
sequenceDiagram
  ...
  PGM->>PGM: read existing PGM, compute this step's MPG and co-emitted TPGs, decrement old-hash entries
  Note over PGM: gate: prior iteration PodGangs must be Available
  PG->>PG: create this step's MPG and TPGs with MinReplicas set and labels stamped
  PCLQ->>PCLQ: take down old-hash standalone pods, recreate schedule-gated against the new MPG
  PCSG->>PCSG: take down old-hash PCSG pods, recreate against the MPG and TPGs of this step (no scheduling gate)
  PCLQ->>PCLQ: remove scheduling gates once PodGang is Initialized and all DependsOn PodGangs are Initialized
  PCS->>PCS: populate InFlightPodGangs with new-hash PodGangs not yet Available
  PG->>PG: pods ready, then release MinReplicas to 0 and set PodGang Available true
  PCS->>PCS: observe Available, clear InFlightPodGangs and explicitly requeue
...
sequenceDiagram
  ...
  PGM->>PGM: rebuild entries from PCLQ and PCSG Status.PodGangMapping, new SPG entries inherit DependsOn selector from current anchor MPG
  PG->>PG: create new SPG from PGM
  PCSG->>PCSG: create pods for new replica (no scheduling gate)
  PG->>PG: pods ready, then release MinReplicas and set Available true
```
...
Coherent updates use a deliberate three-step lifecycle for `MinReplicas` on every PodGang the PodGang component creates — legacy BPG and SPG, MPG, and TPG alike:

1. **Set on creation.** When a PodGang is first materialized from a PGM entry, each `PodGroup`'s `MinReplicas` is set to the `minAvailable` value defined in the PCS spec for that PCLQ or PCSG. This forces the scheduler to place the whole gang at once before any constituent pod can run.
2. **Wait for minAvailable pods to become ready.** The PodGang component tracks each PodGroup against the live pod set and computes whether at least `minAvailable` pods of that group are currently `Ready`.
3. **Release once ready, then mark `Available`.** Once the gang reaches minAvailable-ready, the PodGang component patches `MinReplicas` to 0 on every PodGroup of that PodGang, then sets `Status.Conditions[Type=Available]=True` with `Reason=PodGangAvailable`.

The release step is what allows the orchestrator to advance. Two reasons drive it:

- **The gang-termination floor must drop after placement.** If `MinReplicas` stayed at its initial value, a single pod failure on a settled gang could trigger the backend scheduler's termination path even though the gang is otherwise healthy. Setting `MinReplicas` to 0 after placement means subsequent pod failures or scale-in of PodGroups in that gang no longer risk triggering termination.
...
Each PGM entry carries a `DependsOn *metav1.LabelSelector` selecting sibling PodGangs whose pods must be scheduled before pods in this entry have their scheduling gates removed. A `nil` selector means no dependency. `DependsOn` is the mechanism by which Coherent enforces ordering invariants within a PCS replica:

1. **Within a step**, the MPG is scheduled before any TPGs of the same step.
2. **Across steps**, every PodGang in step `N` is scheduled before any PodGang in step `N+1`.

##### Labels carried by every PodGang

The PodGangMap component populates each PGM entry's `Labels` field (see [`PodGangMap` — new CRD](#podgangmap--new-crd)) with two labels that the PodGang component stamps onto the materialized resource:

| Label | Purpose |
| --- | --- |
| `grove.io/schedule-order` | The step number this PodGang belongs to. Stamped on **every** PodGang the algorithm produces. |
| `grove.io/schedule-anchor` | The step number of which this PodGang is the anchor. Stamped **only on the MPG** of each step. |

##### Selectors authored per PodGang role

| PodGang | `DependsOn` selector |
| --- | --- |
| MPG of step 1 | `nil` — no dependency. |
| MPG of step `N ≥ 2` | `matchLabels: {grove.io/schedule-order: "N-1"}` — every PodGang in the prior step. |
| TPG of step `N` | `matchLabels: {grove.io/schedule-anchor: "N"}` — the MPG of its own step. |

The cross-step ordering (rule 2 above) does not need to be encoded on every PodGang. A TPG in step `N` selects on its step's MPG; that MPG selects on step `N-1`; step `N-1`'s MPG selects on step `N-2`; and so on. The transitive chain through MPG anchors gives the full ordering with each PodGang carrying just one selector.

##### Gate-removal evaluation

The ordering is enforced at gate-removal time, not at PodGang-creation time. When evaluating whether a pod can be ungated, the consumer (PCLQ pod component or PCSG PodClique component) compiles the entry's `DependsOn` via `metav1.LabelSelectorAsSelector` and checks that every matched PodGang reports `Initialized=True`. A nil selector trivially passes.

##### Steady-state scale-out

When the PodGangMap component reconstructs PGM in steady state from PCLQ/PCSG `Status.PodGangMapping`, existing entries' `DependsOn` is preserved. Net-new SPG entries (created by PCSG scale-out) inherit `DependsOn` from the current anchors — they get the same selector that other non-anchor entries carry. Scaling out never accidentally promotes an SPG to anchor status, and never accidentally demotes an existing anchor.
...
Updates to a PCS can be done to a subset of PodCliques or all of the PodCliques. The PodGangMap component runs the [Step-by-step PodGang generation](#step-by-step-podgang-generation) algorithm against the *updated components only*: any standalone PodClique whose template was changed, plus any PodCliqueScalingGroup whose constituent PCLQ template was changed. Components untouched by the update are not in scope and continue to serve traffic in their existing PodGangs.

State just before the update:

```
BPG: {5F, 1P, 1D}, SPG: 3 * {P}, 2 * {D}
PCS replica counts: F = 5, P = 4, D = 3
MinAvailable:       F = 2, P = 1, D = 1
```

The cases below show how the algorithm produces MPGs and TPGs depending on which components are in scope. In each case, old-hash content is drained out of the existing PodGangs into the newly emitted MPGs/TPGs as the update progresses. Let `Fv1`, `Pv1`, `Dv1` denote the new pod-template versions.

**Case #1: All PCLQs (frontend, prefill, decode) are updated**

Components in scope: F (standalone), P (PCSG), D (PCSG). Algorithm inputs:

- `replicas` = (5F, 4P, 3D)
- `minAvailable` = (2F, 1P, 1D)

| Step | `remaining` (F, P, D) | `numMPGs` | `remainder` (P, D) | Emitted | After step (F, P, D) |
|------|------------------------|-----------|---------------------|---------|----------------------|
| 1 | 5, 4, 3 | 2 | 2, 1 | MPG-1{2Fv1, 1Pv1, 1Dv1} + 1×TPG{1Pv1} | 3, 2, 1 |
| 2 | 3, 2, 1 | 1 | 1, 0 | MPG-2{3Fv1, 1Pv1, 1Dv1} + 1×TPG{1Pv1} | 0, 0, 0 |

The 5th F replica is the standalone surplus — it folds into the last MPG (MPG-2 carries 3F instead of 2F).

After the update completes the BPG and SPGs from before T4 have been drained and removed; the replica's PodGangs are MPG-1, MPG-2, and 2 prefill TPGs.

**Case #2: Prefill and Decode (PCSGs) are updated**

Updates to any constituent PCLQ of a PCSG count as an update of the entire PCSG. Components in scope: P (PCSG), D (PCSG). F is **not** in scope and stays in its existing PodGang.

Algorithm inputs (F excluded):

- `replicas` = (4P, 3D)
- `minAvailable` = (1P, 1D)

| Step | `remaining` (P, D) | `numMPGs` | `remainder` (P, D) | Emitted | After step (P, D) |
|------|---------------------|-----------|---------------------|---------|--------------------|
| 1 | 4, 3 | 3 | 1, 0 | MPG-1{1Pv1, 1Dv1} | 3, 2 |
| 2 | 3, 2 | 2 | 1, 0 | MPG-2{1Pv1, 1Dv1} | 2, 1 |
| 3 | 2, 1 | 1 | 1, 0 | MPG-3{1Pv1, 1Dv1} + 1×TPG{1Pv1} | 0, 0 |

While the update runs, F pods stay in the original BPG. As P and D drain out of the BPG into the new MPGs, the BPG's P/D content goes to zero — the BPG eventually becomes a frontend-only PodGang carrying just {5F}.

**Case #3: Only standalone PodClique (frontend) is updated**

Components in scope: F (standalone) only. Algorithm inputs (P and D excluded):

- `replicas` = (5F)
- `minAvailable` = (2F)

| Step | `remaining` (F) | `numMPGs` | `remainder` | Emitted | After step (F) |
|------|------------------|-----------|-------------|---------|-----------------|
| 1 | 5 | 2 | — (no PCSGs in scope) | MPG-1{2Fv1} | 3 |
| 2 | 3 | 1 | — | MPG-2{3Fv1} (standalone surplus folds in) | 0 |

P and D pods stay in their existing PodGangs throughout. As F drains out of the BPG into MPG-1 and MPG-2, the BPG's F content goes to zero — the BPG becomes a {1P, 1D} PodGang. The 3 prefill SPGs and 2 decode SPGs from the pre-update layout are untouched.
...
- **In steady state (status as source of truth):** scale-out adds a new entry to `Status.PodGangMapping` (a SPG name assigned by the PodCliqueScalingGroup reconciler, or an additional bucket on a standalone PCLQ); scale-in zeroes a count. The PodGangMap component reconstructs PGM from these mappings — preserving the `DependsOn` selector on existing entries, having net-new SPG entries inherit the same selector that other non-anchor entries carry, and dropping zero-count entries.
...
- `PodGang.Status.Conditions[Type=Available]` is the gate the orchestrator waits on. `Reason=PodGangAvailable` indicates `MinReplicas` pods of every PodGroup are ready and `MinReplicas` has been released.

**Printer columns on `PodCliqueSet` CRD:** `Replicas`, `Available`, `Updated`, `PCLQs-Updated`, `PCLQs-Total`, `PCSGs-Updated`, `PCSGs-Total` — surface the headline counters in `kubectl get pcs`.
...
- All core behaviors work: MVU iteration, MPG/TPG formation, absorption, gate removal, `Available` signaling, orchestrator advance
