...
- [Proposal](#proposal)
  ...
  - [Step-by-step PodGang generation](#step-by-step-podgang-generation)
    ...
    - [Example 1 — 10F, 11P, 13D, minAvailable = (2F, 2P, 2D)](#example-1--10f-11p-13d-minavailable--2f-2p-2d)
    - [Example 2 — 4F, 100P, 150D, minAvailable = (2F, 2P, 3D)](#example-2--4f-100p-150d-minavailable--2f-2p-3d)
...
- [Design Details](#design-details)
  - [API Changes](#api-changes)
    ...
    - [PodCliqueSetStatus.UpdateProgress extensions for Coherent update strategy](#podcliquesetstatusupdateprogress-extensions-for-coherent-update-strategy)
      ...
      - [Per-replica iteration tracking on PodCliqueSetReplicaUpdateProgress](#per-replica-iteration-tracking-on-podcliquesetreplicaupdateprogress)
  ...
  - [Coherent update behavior and flow](#coherent-update-behavior-and-flow)
    ...
    - [DependsOn and scheduling order](#dependson-and-scheduling-order)
      ...
      - [Pod scheduling-gate removal](#pod-scheduling-gate-removal)
...
- **Coherent update.** One step per iteration. The PodGangMap component computes step `N`'s entries, writes them to PGM, and waits until every PodGang created in that step reports `PodGangConditionTypeReady=True` before emitting step `N+1`.
...
| 4 | 4, 5, 6    | 2 | 1, 2 | MPG{2F, 2P, 2D} + 1×TPG{1D}             | 2, 3, 3  |
| 5 | 2, 3, 3    | 1 | 1, 1 | MPG{2F, 2P, 2D} + 1×TPG{1P} + 1×TPG{1D} | 0, 0, 0  |

Total: 5 MPGs (10F, 10P, 10D) + 1 P-TPG + 3 D-TPGs = 10F, 11P, 13D.
...
type PodCliqueSetReplicaUpdateProgress struct {
    ...
    // PodGangConditionTypeReady=True before advancing to the next iteration.
    // +optional
...
2. The orchestrator reads PGM, picks the new-hash entries that are not yet `PodGangConditionTypeReady`, and writes their names into `InFlightPodGangs`. For a step that emits one MPG and `k` TPGs, this is `1 + k` PodGang names.
3. The orchestrator waits for every listed PodGang to reach `PodGangConditionTypeReady=True`, then clears the field. This signals the PodGangMap component to compute the next step.
...
Grove encodes the MVU's gang-scheduling intent by generating new `PodGang` resources. Each step of an update emits one **MVU PodGang (MPG)** carrying the MVU's pods, plus zero or more **Tail PodGangs (TPGs)**, each carrying a single PCSG replica beyond what the MPGs alone can absorb. The number of MPGs across the update and the number of TPGs co-emitted in each step are determined by the [Step-by-step PodGang generation](#step-by-step-podgang-generation) algorithm. Each MPG is its own gang and each TPG is its own gang — they are scheduled independently. TPGs `DependsOn` the MPG of their own step (via a label selector — see [DependsOn and scheduling order](#dependson-and-scheduling-order)), so the scheduler places TPG pods only after their step's MPG reports `PodGangConditionTypeReady=True`.

A coherent update runs **one step at a time**. The PodGangMap component computes step `N`'s entries (MPG plus any co-emitted TPGs), and the orchestrator waits for every PodGang in step `N` to reach `PodGangConditionTypeReady=True` before step `N+1` is computed. Disruption is bounded by the step shape — at most one step's worth of new-hash pods is in flight at any moment.
...
7. **Advance.** Once all in-flight PodGangs have their `PodGangConditionTypeReady` condition set to `True`, the orchestrator clears `InFlightPodGangs` and explicitly requeues (the status-only patch does not fire a new reconcile on its own since the PCS For-watch uses `GenerationChangedPredicate`). On the next reconcile the PodGangMap component computes the next iteration's entries (step 2). When no old-hash entries remain for the replica, the orchestrator marks the replica's `UpdateEndedAt` and seeds `CurrentlyUpdating[0]` for the next pending replica. When all replicas are done, `UpdateProgress.UpdateEndedAt` is set and the strategy enters steady state.
...
An iteration corresponds to one step of the [Step-by-step PodGang generation](#step-by-step-podgang-generation) algorithm. The PodGangMap component computes the step's MPG and any co-emitted TPGs together, and the orchestrator gates the next iteration on every PodGang in this step reaching `PodGangConditionTypeReady=True`.
...
sequenceDiagram
  ...
  PG->>PG: scheduler places MinReplicas pods, then release MinReplicas and set PodGang Scheduled true
  PG->>PG: pods become Ready (pass readiness probes), then set PodGang Ready true
```
...
| Condition | Meaning |
| --- | --- |
| `PodGangConditionTypeScheduled` | Set to `True` once `MinReplicas` pods of every `PodGroup` have been scheduled onto nodes. Setting `PodGangConditionTypeScheduled=True` also implies `MinReplicas` has been released to 0 on all PodGroups. |
| `PodGangConditionTypeReady` | Set to `True` once `MinReplicas` pods of every `PodGroup` are `Ready` (passing readiness probes). |

These conditions are progressive: a PodGang reaches `PodGangConditionTypeScheduled=True` first, then `PodGangConditionTypeReady=True`. Both stay `True` for the rest of the PodGang's lifetime — they are not flipped back to `False` after, for example, a pod failure or scale-in.
...
2. **Release `MinReplicas`, mark `PodGangConditionTypeScheduled=True`.** Once the scheduler has placed `MinReplicas` pods of every `PodGroup` on nodes, the PodGang component patches `MinReplicas=0` on every PodGroup, then sets `Status.Conditions[Type=Scheduled]=True` with `Reason=PodGangScheduled`. Pod-component gate-removal logic uses this condition (see [`DependsOn` and scheduling order](#dependson-and-scheduling-order)).
3. **Mark `PodGangConditionTypeReady=True`.** Once `MinReplicas` pods of every `PodGroup` pass readiness probes, the PodGang component sets `Status.Conditions[Type=Ready]=True` with `Reason=PodGangReady`. The orchestrator (PodCliqueSet reconciler) uses this condition to advance coherent-update steps (see [Per-replica iteration tracking on `PodCliqueSetReplicaUpdateProgress`](#per-replica-iteration-tracking-on-podcliquesetreplicaupdateprogress)).

`MinReplicas` is released at stage 2 — when the scheduler has done its job — rather than at stage 3. The reason: `MinReplicas` is a *scheduler* directive (gang-placement floor and, on some backends, gang-termination floor); pod readiness is the *workload's* concern, not the scheduler's. Holding `MinReplicas` until pods pass readiness probes would conflate the two and would risk the backend scheduler terminating the gang on any pod failure that occurs *before* the gang reaches readiness — a single pod crash on an otherwise-placed gang could trigger termination before the workload has even had a chance to start. Releasing `MinReplicas` at `PodGangConditionTypeScheduled=True` decouples the two concerns: once placement is done, readiness is purely a workload signal that the orchestrator (and only the orchestrator) consults.

The same lifecycle applies in steady state to PodGangs created by PCSG scale-out — the new SPG follows set → release + `PodGangConditionTypeScheduled=True` → `PodGangConditionTypeReady=True` exactly as an MPG does during an update.
...
##### Pod scheduling-gate removal

Each pod created against a `DependsOn`-bearing PGM entry starts with a scheduling gate (`pod.Spec.SchedulingGates`) so the scheduler will not place it. The gate is removed by the consumer (PCLQ pod component or PCSG PodClique component): it compiles the entry's `DependsOn` via `metav1.LabelSelectorAsSelector` and removes the gate once every matched PodGang reports `PodGangConditionTypeScheduled=True`. A nil selector trivially passes.

| Flow | Gate for ungating step `N+1` pods (pod-component reads) | Gate for emitting step `N+1` PodGangs (orchestrator reads) |
| --- | --- | --- |
| **Initial deployment** | Step `N`'s PodGangs report `PodGangConditionTypeScheduled=True` | n/a — all step entries are written to PGM in one shot at deploy time |
| **Coherent update** | Step `N`'s PodGangs report `PodGangConditionTypeScheduled=True` | Step `N`'s PodGangs report `PodGangConditionTypeReady=True` |

During a **coherent update** the orchestrator waits for the previous step to be fully `Ready` (workload serving) before emitting the next step's PodGangs — this preserves bounded disruption while old-hash pods are taken down. During **initial deployment** the algorithm writes every step's entries up front, so there is no orchestrator-side per-step gate; pod ungating happens as each step's PodGangs reach `PodGangConditionTypeScheduled=True`, and the rollout is not paced by readiness probes between steps.
...
- `PodGang.Status.Conditions[Type=Scheduled]` is set to `True` when the scheduler has placed `MinReplicas` pods of every PodGroup; `Reason=PodGangScheduled`. This condition gates pod scheduling-gate removal across both flows.
- `PodGang.Status.Conditions[Type=Ready]` is set to `True` when `MinReplicas` pods of every PodGroup are passing readiness probes; `Reason=PodGangReady`. The orchestrator waits on this condition before advancing coherent-update steps.
...
- All core behaviors work: MVU iteration, MPG/TPG formation, absorption, gate removal, `PodGangConditionTypeReady` signaling, orchestrator advance
