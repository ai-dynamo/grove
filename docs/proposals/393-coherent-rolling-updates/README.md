...
* Allowing scale operations (`PodClique`, `PodCliqueScalingGroup`, `PodCliqueSet`) during an in-flight coherent update. The current iteration rejects scale-out and scale-in PCS-wide while a coherent update is in flight. See [Handling scale-outs and scale-ins during update](#handling-scale-outs-and-scale-ins-during-update). Narrower scoping will be supported in future iterations.
