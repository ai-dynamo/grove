...
**An authorized release leaves the PodClique below `minAvailable`, which arms gang termination.** This is the sharpest open problem in the proposal. `minAvailable` defaults to the clique template's `replicas` and is the gang-admission minimum published to the scheduler as `PodGang.MinReplicas`, so full-width atomic admission implies `minAvailable == replicas`. It is immutable across PodCliqueSet updates and is not lowered by a `/scale` write. So for the motivating case — an 8-replica engine clique authorized down to 7 — the release succeeds, `scheduledReplicas (7) < minAvailable (8)` sets `MinAvailableBreached=True`, and after `terminationDelay` the PCSG controller gang-terminates the **entire PCSG replica**: every member clique of that engine world, not just the released pod. The authorized scale-in would destroy the world it was protecting.

Two mitigations, neither free. The workload can pre-set `minAvailable` to the lowest size it ever intends to run at, which gives up full-width atomic admission — precisely the distinction #793 raises and this GREP lists as a Non-Goal. Or `minAvailable` gains the initial-admission/runtime-minimum split that #793 asks for, which is a change to gang semantics for every Grove user. **This proposal is not independently shippable without one of them**, and that is a stronger coupling to the `minAvailable` Non-Goal than the Non-Goals section currently admits.
...
One semantic overlap is open rather than resolved, and is called out here so a reviewer does not have to find it. [GREP-0677](https://github.com/ai-dynamo/grove/pull/686) (draft, unmerged) states under *Zero Replicas Per Level* that **"a `PodClique` owned by a `PodCliqueScalingGroup` must not be scaled independently, including to zero"**, and proposes rejecting `0 < replicas < minAvailable` on create, on update, and through the `/scale` subresource. This GREP assumes the opposite for the non-zero case: that resizing a member clique while its `PodCliqueScalingGroup` stays at a fixed positive `replicas` is a supported operation. That is also what [#793](https://github.com/ai-dynamo/grove/issues/793) asks maintainers to confirm — whether direct scale updates to PCSG-owned PodCliques are intended as a stable integration contract for an external controller.

The two do not conflict in code today, only in stated contract: Grove currently preserves a live member-clique replica count rather than resetting it to the template, and no admission webhook covers `PodClique` at all. But if GREP-0677's rule is adopted as written, it forecloses the mechanism this GREP is built on. **Resolving that is a prerequisite for accepting this proposal**, and the resolution is a one-line distinction — member cliques may be resized within a live scaling group; only scale-to-zero is expressed at the group level.
