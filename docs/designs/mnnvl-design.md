...
1. Does the PCS have `grove.io/mnnvl-enabled: "true"`?
...
When the PCS controller creates a PCSG, it propagates the `grove.io/mnnvl-enabled` annotation to the PCSG:
...
metadata:
  ...
  annotations:
    grove.io/mnnvl-enabled: "true"  # Propagated from PCS
  labels:
    app.kubernetes.io/part-of: my-pcs
    grove.io/podcliqueset-replica-index: "0"
spec:
...
1. Check if PCSG has `grove.io/mnnvl-enabled: "true"` annotation
