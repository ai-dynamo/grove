...
This guide will walk you through deploying a simple multi-component AI workload using Grove in about 10 minutes.
...
We'll deploy a simple workload that mimics a disaggregated inference setup with four components. The role names are inference-flavored, but the same Grove primitives can model training and hybrid pipelines translated by controllers, adapters, or platform layers from systems such as Ray, Dynamo, llm-d, NeMo-RL, Megatron-LM, or Kubeflow Trainer.
