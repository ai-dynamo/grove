...
We'll deploy a simple workload that mimics a disaggregated inference setup with four components. The role names are inference-flavored, but the same Grove primitives can model training and hybrid pipelines generated by frameworks such as Ray, Dynamo, llm-d, NeMo-RL, Megatron-LM, or Kubeflow Trainer.
