...
| Field | Description | Default | Validation |
| --- | --- | --- | --- |
| `required` _string_ | Required defines topology constraint that must be satisfied and is usually less strict than Preferred.<br />It serves as an upper bound for topology packing.<br />Holds topologyKey (not level name) translated from user's packLevel specification.<br />Example: "topology.kubernetes.io/rack" |  |  |
| `preferred` _string_ | Preferred defines best-effort topology constraint. It is usually a stricter constraint<br />than the one that is defined in Required and serves as a lower bound for topology packing.<br />Since it is preferred, it is therefore not binding on the scheduler to mandatorily satisfy this packing constraint.<br />Scheduler can fall back to less strict levels (upto Required constraint) if preferred cannot be satisfied.<br />Example: "kubernetes.io/hostname" |  |  |
