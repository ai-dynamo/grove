...
type TopologyPackConstraint struct {
	// Required defines topology constraint that must be satisfied and is usually less strict than Preferred.
	// It serves as an upper bound for topology packing.
	...
	// Preferred defines best-effort topology constraint. It is usually a stricter constraint
	// than the one that is defined in Required and serves as a lower bound for topology packing.
	// Since it is preferred, it is therefore not binding on the scheduler to mandatorily satisfy this packing constraint.
	// Scheduler can fall back to less strict levels (upto Required constraint) if preferred cannot be satisfied.
