...
//
//	./operator/hack/infra-manager.py
//
...
//
// For local development with k3d, run './operator/hack/infra-manager.py' first
...
func getRestConfig() (*rest.Config, error) {
	...
	if kubeconfigPath == "" {
		return nil, fmt.Errorf("failed to get kubernetes config: no KUBECONFIG found and ~/.kube/config not accessible." +
			"For local development, run './operator/hack/infra-manager.py' first")
	}
