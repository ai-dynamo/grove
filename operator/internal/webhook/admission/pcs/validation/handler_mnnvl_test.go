...
func TestValidateCreate_MNNVL(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			pcs:              createValidPCSWithGPU(map[string]string{mnnvl.AnnotationAutoMNNVL: "true"}),
		...
		},
		{
			...
			pcs:              createValidPCSWithGPU(map[string]string{mnnvl.AnnotationAutoMNNVL: "false"}),
			autoMNNVLEnabled: false,
			expectError:      false,
		},
		{
...
func TestValidateUpdate_MNNVL(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			oldPCS:      createValidPCSWithGPU(map[string]string{mnnvl.AnnotationAutoMNNVL: "true"}),
			newPCS:      createValidPCSWithGPU(map[string]string{mnnvl.AnnotationAutoMNNVL: "true"}),
			expectError: false,
		},
		{
		...
		},
		{
			...
			oldPCS:        createValidPCSWithGPU(map[string]string{mnnvl.AnnotationAutoMNNVL: "true"}),
			newPCS:        createValidPCSWithGPU(map[string]string{mnnvl.AnnotationAutoMNNVL: "false"}),
			expectError:   true,
			errorContains: "immutable",
		},
		{
			description:   "annotation changed false to true -> error",
			oldPCS:        createValidPCSWithGPU(map[string]string{mnnvl.AnnotationAutoMNNVL: "false"}),
			newPCS:        createValidPCSWithGPU(map[string]string{mnnvl.AnnotationAutoMNNVL: "true"}),
			expectError:   true,
			errorContains: "immutable",
		},
