...
func (r _resource) createOrUpdatePCLQs(ctx context.Context, logger logr.Logger, pcs *grovecorev1alpha1.PodCliqueSet, existingPCLQFQNs []string) error {
	...
	if err != nil {
		return groveerr.WrapError(err,
			errSyncPodClique,
			component.OperationSync,
			fmt.Sprintf("error getting current revision for PodCliqueSet %v", client.ObjectKeyFromObject(pcs)),
		)
	}
	...
	for pcsReplicaIndex := range pcs.Spec.Replicas {
		...
		for _, expectedPCLQName := range expectedPCLQNames {
			...
			podTemplateHash, err := revision.CliqueHash(expectedPCLQName)
			if err != nil {
				return err
			}
			...
			createOrUpdateTask := utils.Task{
				...
				Fn: func(ctx context.Context) error {
					return r.doCreateOrUpdate(ctx, logger, pcs, pcsReplicaIndex, pgm, pclqObjectKey, pclqExists, podTemplateHash)
				},
			}
...
func (r _resource) doCreateOrUpdate(ctx context.Context, logger logr.Logger, pcs *grovecorev1alpha1.PodCliqueSet, pcsReplica int32, pgm *grovecorev1alpha1.PodGangMap, pclqObjectKey client.ObjectKey, pclqExists bool, podTemplateHash string) error {
	...
	opResult, err := controllerutil.CreateOrPatch(ctx, r.client, pclq, func() error {
		return r.buildResource(logger, pcs, int(pcsReplica), pclqExists, pgm, pclq, podTemplateHash)
	})
	if err != nil {
...
func (r _resource) buildResource(logger logr.Logger, pcs *grovecorev1alpha1.PodCliqueSet, pcsReplica int, pclqExists bool, pgm *grovecorev1alpha1.PodGangMap, pclq *grovecorev1alpha1.PodClique, podTemplateHash string) error {
	var err error
	...
	pclq.Labels = getLabels(pcs, pcsReplica, pclqObjectKey, pclqTemplateSpec, podGangName, podTemplateHash)
