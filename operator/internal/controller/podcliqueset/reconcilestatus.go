...
func (r *Reconciler) reconcileStatus(ctx context.Context, logger logr.Logger, pcs *grovecorev1alpha1.PodCliqueSet) ctrlcommon.ReconcileStepResult {
	...
	err = r.mutateReplicas(ctx, logger, selectedRevision, pcs)
	if err != nil {
...
func (r *Reconciler) mutateReplicas(ctx context.Context, logger logr.Logger, revision *commonrevision.SelectedRevision, pcs *grovecorev1alpha1.PodCliqueSet) error {
	// Set basic replica count
	pcs.Status.Replicas = pcs.Spec.Replicas
	stats, err := r.computeAvailableAndUpdatedReplicas(ctx, logger, revision, pcs)
	if err != nil {
	...
	if shouldCompleteAutoUpdate(pcs, stats) {
		logger.Info("Rolling update has completed from aggregate child status")
		completedAt := metav1.Now()
		pcs.Status.UpdateProgress.UpdateEndedAt = &completedAt
...
// shouldCompleteAutoUpdate makes aggregate status the sole owner of automatic rollout
// completion. Every predicate is derived from the same child observation as the counts that
// will be persisted with UpdateEndedAt.
func shouldCompleteAutoUpdate(pcs *grovecorev1alpha1.PodCliqueSet, stats pcsReplicaStats) bool {
	progress := pcs.Status.UpdateProgress
	return componentutils.IsAutoUpdateStrategy(pcs) &&
		progress != nil && progress.UpdateEndedAt == nil &&
		stats.updatedReplicas == pcs.Spec.Replicas &&
...
func (r *Reconciler) computeAvailableAndUpdatedReplicas(ctx context.Context, logger logr.Logger, revision *commonrevision.SelectedRevision, pcs *grovecorev1alpha1.PodCliqueSet) (pcsReplicaStats, error) {
	var (
...
func countUpdatedPCLQs(revision *commonrevision.SelectedRevision, pclqs []grovecorev1alpha1.PodClique) int32 {
	var n int32
	for i := range pclqs {
		pclq := &pclqs[i]
		...
		if isStandalonePCLQUpdated(revision, pclq) {
			n++
		}
	}
	return n
...
func (r *Reconciler) computeReplicaStatus(revision *commonrevision.SelectedRevision, replicaPCSGs []grovecorev1alpha1.PodCliqueScalingGroup, standalonePCLQs []grovecorev1alpha1.PodClique, expectedPCSGs int, expectedStandalonePCLQs int) (bool, bool) {
...
func (r *Reconciler) computePCLQsStatus(revision *commonrevision.SelectedRevision, expectedStandalonePCLQs int, existingPCLQs []grovecorev1alpha1.PodClique) (isAvailable, isUpdated bool) {
	...
	isUpdated = isAvailable && lo.EveryBy(nonTerminatedPCLQs, func(pclq grovecorev1alpha1.PodClique) bool {
		return isStandalonePCLQUpdated(revision, &pclq)
	})
...
func isStandalonePCLQUpdated(revision *commonrevision.SelectedRevision, pclq *grovecorev1alpha1.PodClique) bool {
