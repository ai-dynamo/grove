...
func (r *Reconciler) processUpdate(ctx context.Context, logger logr.Logger, pclq *grovecorev1alpha1.PodClique) ctrlcommon.ReconcileStepResult {
	...
	podTemplateHash, err := componentutils.GetExpectedPCLQPodTemplateHash(revision, pclq.ObjectMeta)
	if err != nil {
	...
	if !componentutils.IsAutoUpdateStrategy(pcs) {
		if shouldResetOrTriggerUpdate(revision.GenerationHash(), pclq) {
			if err = r.initOrResetUpdate(ctx, pcs, pclq, revision.GenerationHash(), podTemplateHash); err != nil {
	...
	if shouldResetOrTriggerUpdate(revision.GenerationHash(), pclq) {
		logger.Info("PodCliqueSet has a new generation hash. Initializing or resetting update for PodClique", "PodCliqueSetGenerationHash", revision.GenerationHash(), "CurrentPodCliqueSetGenerationHash", pclq.Status.CurrentPodCliqueSetGenerationHash, "isPCLQAutoUpdateInProgress", componentutils.IsPCLQAutoUpdateInProgress(pclq), "isLastPCLQUpdateCompleted", componentutils.IsLastPCLQUpdateCompleted(pclq))
		if err = r.initOrResetUpdate(ctx, pcs, pclq, revision.GenerationHash(), podTemplateHash); err != nil {
...
func shouldResetOrTriggerUpdate(generationHash string, pclq *grovecorev1alpha1.PodClique) bool {
	...
	firstEverUpdateRequired := pclq.Status.UpdateProgress == nil && pclq.Status.CurrentPodCliqueSetGenerationHash != nil && *pclq.Status.CurrentPodCliqueSetGenerationHash != generationHash
	...
	inProgressPCLQUpdateNotStale := componentutils.IsPCLQAutoUpdateInProgress(pclq) && pclq.Status.UpdateProgress.PodCliqueSetGenerationHash == generationHash
	// PCLQ had an update in the past but that was for an older PCS generation hash.
	lastCompletedUpdateIsNotStale := componentutils.IsLastPCLQUpdateCompleted(pclq) && pclq.Status.UpdateProgress.PodCliqueSetGenerationHash == generationHash
...
func (r *Reconciler) initOrResetUpdate(ctx context.Context, pcs *grovecorev1alpha1.PodCliqueSet, pclq *grovecorev1alpha1.PodClique, generationHash, podTemplateHash string) error {
