...
var (
	...
	// ErrTopologyNameMismatch indicates that a child topologyName differs from the PCS-level topologyName.
	ErrTopologyNameMismatch = errors.New("child topologyName must match pcs-level topologyName")
)
...
// HasChildTopologyNameMismatch reports whether any child resource sets topologyName to a value
// different from the PCS-level topologyName.
func HasChildTopologyNameMismatch(pcs *grovecorev1alpha1.PodCliqueSet, pcsTopologyName string) bool {
	for _, clique := range pcs.Spec.Template.Cliques {
		if clique.TopologyConstraint != nil &&
...
// GetChildTopologyNameMismatchPaths returns field paths for all child topologyName fields
// whose values differ from the PCS-level topologyName.
func GetChildTopologyNameMismatchPaths(pcs *grovecorev1alpha1.PodCliqueSet, pcsTopologyName string) []string {
	paths := sets.New[string]()
	for i, clique := range pcs.Spec.Template.Cliques {
		if clique.TopologyConstraint != nil &&
			clique.TopologyConstraint.TopologyName != "" &&
			clique.TopologyConstraint.TopologyName != pcsTopologyName {
			paths.Insert(fmt.Sprintf("spec.template.cliques[%d].topologyConstraint.topologyName", i))
		}
	}
	for i, pcsg := range pcs.Spec.Template.PodCliqueScalingGroupConfigs {
		if pcsg.TopologyConstraint != nil &&
			pcsg.TopologyConstraint.TopologyName != "" &&
			pcsg.TopologyConstraint.TopologyName != pcsTopologyName {
			paths.Insert(fmt.Sprintf("spec.template.podCliqueScalingGroups[%d].topologyConstraint.topologyName", i))
		}
	}
...
func ResolveTopologyNameForPodCliqueSet(pcs *grovecorev1alpha1.PodCliqueSet) (string, error) {
	...
	if HasChildTopologyNameMismatch(pcs, topologyName) {
		return "", ErrTopologyNameMismatch
	}
